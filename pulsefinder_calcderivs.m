...
Uforward = zeros(params.HilbertDim,params.HilbertDim,plength);
Uforward(:,:,1) = prop(:,:,1);
for ct = 2:1:plength
...
Uback = zeros(params.HilbertDim,params.HilbertDim,plength);

%Whether we are doing unitary or state this is a bit different
if(params.searchtype == 1)
    Uback(:,:,1) = -Uwant;
else
    Uback(:,:,1) = eye(params.HilbertDim);
end
...
if(params.searchtype == 1) %Do the derivatives for unitary
    tmpRFmat = zeros(size(RFmatts,3),params.HilbertDim^2);
    
    for RFmattct = 1:1:size(RFmatts,3)
        tmpRFmat(RFmattct,:) = reshape(transpose(RFmatts(:,:,RFmattct)),[1 params.HilbertDim^2]);
    end
    
    for ct = 1:1:plength
        tmpmat = Uforward(:,:,ct)*Uback(:,:,ct)';
        tmpmult = conj(trace(tmpmat));
        tmpmatt = reshape(tmpmat,[1 params.HilbertDim^2]);
...
%            derivs(ct,RFmattct+1) = -2*real(trace(Uback(:,:,ct)'*i*pulse(ct,1)*RFmatts(:,:,RFmattct)*Uforward(:,:,ct))*trace(Uforward(:,:,ct)'*Uback(:,:,ct))); 
        ...
        if(params.tstepflag)
            derivs(ct,1) = 2*imag(sum(reshape(transpose(HTOT(:,:,ct)),[1 params.HilbertDim^2]).*tmpmatt)*tmpmult);
        end
...
if(nargout == 3)
    ...
    if(params.searchtype == 1)
        ...
        for ct = 1:1:params.nbspins
            newgoodness = (abs(sum(exp(-1i*pi*1e-6*corrmat(:,ct)).*tmpmat)))^2;
        ...
        for ct = 1:1:params.nbspins
            newgoodness = (abs(trace(rhogoal'*Usim*diag(exp(-1i*pi*1e-6*corrmat(:,ct)))*rhoin*diag(exp(1i*pi*1e-6*corrmat(:,ct)))*Usim')))^2;
            zderivs(1,ct) = 1e6*(newgoodness-goodness);
        end

        %And now Zpost
        for ct = 1:1:params.nbspins
            newgoodness = (abs(trace(rhogoal'*diag(exp(-1i*pi*1e-6*corrmat(:,ct)))*Usim*rhoin*Usim'*diag(exp(1i*pi*1e-6*corrmat(:,ct))))))^2;
            zderivs(2,ct) = 1e6*(newgoodness-goodness);
        end
    end %params.searchtype if
end %nargout == 3 if